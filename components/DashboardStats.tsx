...
export const DashboardStats: React.FC<Props> = ({ deliveries, historyData }) => {
  ...
  // Grouping W1 (Kurier) with Unloading (W5-W8) based on requirement "ROZŁADUNKI (W1, W5-W8)"
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden shrink-0 font-sans flex h-28">
      {/* LEFT PANEL: SPECIFIC STATS */}
      <div className="w-1/2 lg:w-3/5 flex flex-col border-r border-gray-200">
          {/* SECTION: ZAŁADUNKI (LOADING) */}
          ...
          <div className="h-px bg-gray-100 mx-0"></div>

          {/* SECTION: ROZŁADUNKI (UNLOADING) */}
          <div className="flex-1">
            <StatRow 
                label="ROZŁADUNKI"
                subLabel="(W1, W5-W8)"
      ...
      <div className="w-1/2 lg:w-2/5 p-2 bg-gray-50 flex flex-col justify-center relative">
         ...
         <ResponsiveContainer width="100%" height="100%">
            <LineChart data={historyData} margin={{ top: 15, right: 10, left: -25, bottom: 0 }}>
               ...
               <YAxis 
                ...
                />
               <Tooltip 
                    contentStyle={{ borderRadius: '4px', fontSize: '11px', padding: '4px' }}
                    itemStyle={{ padding: 0 }}
               />
...
const StatRow: React.FC<StatRowProps> = ({ label, subLabel, stats, colorTheme }) => {
    const themeClasses = colorTheme === 'blue' 
        ? { text: 'text-blue-600', bg: 'bg-blue-50/50', border: 'border-blue-100', icon: 'text-blue-500' }
        : { text: 'text-emerald-600', bg: 'bg-emerald-50/50', border: 'border-emerald-100', icon: 'text-emerald-500' };

    return (
        <div className="flex items-stretch h-full">
            {/* LEFT LABEL */}
            <div className={`w-36 shrink-0 flex flex-col justify-center items-center ${themeClasses.bg} border-r border-gray-100`}>
            ...
            <div className="flex-grow flex items-center justify-between px-4 lg:px-8">
                ...
                <StatItem 
                    value={stats.pending} 
                    label="OCZEKUJE" 
                    icon={<svg className="w-3 h-3 text-amber-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>}
                />
                ...
                <StatItem 
                    value={stats.onSite} 
                    label="NA PLACU" 
                    isLive={stats.onSite > 0}
                    icon={<svg className={`w-3 h-3 ${themeClasses.icon}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>}
                />
                ...
                <StatItem 
                    ...
                    label="ZAKOŃCZONE" 
                    icon={<svg className="w-3 h-3 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>}
                />
                 ...
                 <StatItem 
                    value={stats.total} 
                    label="WSZYSTKIE" 
                    icon={<svg className="w-3 h-3 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" /></svg>}
                />
...
const StatItem = ({ value, label, icon, isLive }: { value: number, label: string, icon: React.ReactNode, isLive?: boolean }) => (
    <div className="flex flex-col items-center justify-center w-16 lg:w-20">
        <div className="flex items-center gap-1.5 mb-0.5">
             <span className={`text-sm lg:text-base font-bold ${value === 0 ? 'text-gray-300' : 'text-slate-700'}`}>{value}</span>
